<x-app-layout>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>Transcript for {{ $student[0]->getUser->name }} - ({{ $student[0]->getUser->email }}) - ({{ $student[0]->getUser->hours }})</h1>
        </div><!-- End Page Title -->
                            ...
                            <table class="table table-borderless datatable">
                                ...
                                @foreach($student as $level1 => $key)
                                    <tr>
                                        ...
                                        <td><a href="{{route('admin.st',['id'=>$key->user_id,'sub'=>$key->sub1])}}">{{($key->sub1)}}</a></td>
                                        <td><a href="{{route('admin.st',['id'=>$key->user_id,'sub'=>$key->sub2])}}">{{($key->sub2)}}</a></td>
                                        <td><a href="{{route('admin.st',['id'=>$key->user_id,'sub'=>$key->sub3])}}">{{($key->sub3)}}</a></td>
                                        <td><a href="{{route('admin.st',['id'=>$key->user_id,'sub'=>$key->sub4])}}">{{($key->sub4)}}</a></td>
                                        <td><a href="{{route('admin.st',['id'=>$key->user_id,'sub'=>$key->sub5])}}">{{($key->sub5)}}</a></td>
                                        <td><a href="{{route('admin.st',['id'=>$key->user_id,'sub'=>$key->sub6])}}">{{($key->sub6)}}</a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
